...
######################################################################################   
        ...
        with col1:
            ...
            fig = go.Figure()
            fig.add_trace(go.Box(y= Age_DF_clean["Age"], x = Age_DF_clean[Age_DF_clean['category'].str.contains('actor')], name = 'Acteurs', marker_color='royalblue', boxmean=True # represent mean
            ))
            fig.add_trace(go.Box(y=Age_DF_clean["Age"], x= Age_DF_clean[Age_DF_clean['category'].str.contains('actress')], name = 'Actrices', marker_color='coral', boxmean=True # represent mean
            ))
            #fig.update_yaxes(title= 'Age')
            #fig.update_xaxes(title= 'Population')
            #fig.update_yaxes(title= 'Age')
